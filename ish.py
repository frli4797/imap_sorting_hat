...
class ISH:
    ...
    def connect_noninteractive(self) -> bool:
        ...
        if not self.connect_imap():
            self.logger.error(
                f"Failed to connect to imap server. Configure, or check your settings in \
        ...
        if not self.connect_openai():
            self.logger.error(
                f"Failed to connect to openai. Configure or check your settings in \
                    {self.settings.settings_file}"
            )
            return False
    ...
    def learn_folders(self, folders: List[str]) -> RandomForestClassifier:
        ...
        t1 = perf_counter()
        self.logger.info(
            f"Fetched {len(embed_array)} embeddings in {t1-t0:.2f} seconds"
        )
        ...
        if inference_mode:
            ...
            while opt not in ["y", "n", "q"]:
                ...
                if opt == "y":
                    ...
                    self.logger.info(
                        f"Quitting. Finished moved {moved} and skipped {skipped}"
                    )
                    exit(0)
                elif opt == "n":
                    skipped += 1
